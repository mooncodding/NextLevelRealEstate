<template>
  <v-app>
    <div class="content-wrapper">
      <div v-if="is('Super Admin') || can('properties')">
        ...
        <div class="content-header">
          <div class="container-fluid">
            <div class="row">
              ...
              <!-- /.col -->
              <div class="col-6">
                <button class="btn btn-success float-right" @click="addProperty" v-if="(is('Super Admin') || can('create_propertie'))">
                  Create Property
              ...
              <div class="col-12">
                <div class="card">
                  ...
                  <div class="card-body table-responsive p-0">
                    <v-data-table
                      :headers="headers"
                      :items="properties.data ? properties.data : properties"
                    ...
                    >
                      <template v-slot:item.actions="{ item }">
                        ...
                        <v-icon
                          ...
                          v-if="(is('Super Admin') || can('edit_propertie'))"
                        >
                          mdi-pencil
                        </v-icon>
                        ...
                        <v-icon
                          ...
                          v-if="(is('Super Admin') || can('delete_propertie'))"
                        >
                          mdi-delete
                        </v-icon>
                      </template>
        ...
        <!-- /.content -->
        <addEditPropertyModal></addEditPropertyModal>
        <viewPropertyModal :propertieData="propertieDataInfo"></viewPropertyModal>
      </div>
...
<script>
import addEditPropertyModal from "./modals/addEditPropertyModal.vue";
...
export default {
  ...
  methods: {
    getResults(page = 1, rows = 10, sortBy = null, sortDesc = null) {
      if (this.is('Super Admin') || this.can('properties')) {
        this.$Progress.start();
        this.loading = true;
        this.curpage = page;
    ...
    deleteProperty(id) {
      if (this.is('Super Admin') || this.can('delete_propertie')) {
        swal
          .fire({
          ...
          .then((result) => {
            ...
            if (result.value) {
              ...
              this.form
                .delete("api/adminPropertys/" + id)
                .then(() => {
                  this.properties.total -= 1;
                  Fire.$emit("reloadPropertys");
                  this.$Progress.finish();
                  swal.fire(
    ...
    addProperty() {
      if (this.is('Super Admin') || this.can('create_propertie')) {
        $("#addEditPropertyModal").modal("show");
      }else{
        toast.fire({
          icon: "error",
          title: this.$t("message.UNAUTHORIZED")
    ...
    editProperty(propertieData) {
      if (this.is('Super Admin') || this.can('edit_propertie')) {
        this.propertieDataInfo = propertieData;
        $("#addEditPropertyModal").modal("show", propertieData);
      }else{
        toast.fire({
          icon: "error",
          title: this.$t("message.UNAUTHORIZED")
    ...
    viewProperty(propertieData) {
      this.propertieDataInfo = propertieData;
      $("#viewPropertyModal").modal("show", propertieData);
    },
  },
  created() {
    Fire.$on("reloadPropertys", () => {
